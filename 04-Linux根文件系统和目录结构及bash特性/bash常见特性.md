...
## 6. globbing - 文件名通配
通配符指的是 bash 中的特殊字符，称为元字符，可用于表示一类或一组字符。通过元字符达到模糊匹配的作用。需要注意的是通配符跟正则表达式是完全不同的东西，不同软件，程序中的通配符也不会完全相同。bash 中的元字符如下
- `*`：匹配任意长度的任意字符
- `?`：匹配任意单个字符
...
## 7. IO重定向及管道
### 7.1 管道
管道是 Linux 提供的一种进程间交互(IPC)的一种方法。大家不必过于纠结它是个什么东西，只要知道的是，它可以把一个程序的输入编程另一个程序的输入，就像一根管道一样连接着程序与程序。这方面有如下概念:
...
### 7.2 IO重定向：
IO 我的理解就是文本流，所谓 IO 重定向就是将原本输入一个地方的文件流重新导向另一个地方。IO 重定可分为覆盖重定向和追加重定向，所谓覆盖就是如果重定向的目标是文件，会先清空文件中的内容，而追加只是在文件的结尾继续写入。IO实现如下
1. 输入重定向：
	- `<`
1. 输出重定向
	- `>`: 覆盖重定向 == `1>`
...
需要注意的是上述中 1,2 指代的是标准输入输出对应的文件描述符号。文件描述符是操作系统的一个抽象概念，表示打开的文件。大家可以理解为 Linux中一切皆文件，如果要操作文件必须将文件关联到某个文件描述符。bash 在开启时，会自动做如下关联
- 标准输入：0
- 标准输出：1
- 错误输出：2
