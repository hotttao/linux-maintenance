...
上图是 yum 架构的示意图，yum 是 C/S 架构的服务。Serve 是一个文件服务器，称为 yum 仓库或 yum 源，包含了众多 rpm 包以及包相关的元数据文件。元数据放置在 repodata 目录，其中包含了包之间的依赖关系。
...
#### yum Server
1. yum 服务器端支持 `ftp://，http://，nfs://，file://` 四种协议 `file://` 表示本地文件系统
...
3. 包含 repodata 目录的路径，就是 yum 源应该指向的路径
...
|变量名称|作用|
|：---|：---|
|$releasever|当前OS的发行版的主版本号；Centos6，即为6，Centos7 即为 7|
|$arch|平台，比如 i386，x86, x86_64|
...
- 作用: 查找程序包，会以指定的关键字搜索程序包名及summary信息；
