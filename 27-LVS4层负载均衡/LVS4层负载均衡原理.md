...
#### LVS
LVS 又称为四层路由器，四层交换机，位于内核空间，直接附加在 iptables netfilter 的 nat 表的 INPUT 链上。可直接根据请求报文的目标 IP 和 port 向后端服务器转发报文，无需创建套接字，因此没有套接字数量的限制。
...
2. LVS 提供的功能有限，并没有 nninx/haproxy 方便
3. 当企业的并发请求超过套接子的限制时，更加倾向于通过硬件实现负载均衡。

但是 LVS 仍然不失为高并发下负载均衡的有效解决方案，而且学习 LVS有助于我们深入理解负载均衡，网络协议等知识，同时 LVS 也是面试重点，因此我们还是要学好 LVS。
